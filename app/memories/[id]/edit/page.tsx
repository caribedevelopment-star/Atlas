...
export default function EditMemoryPage() {
  ...
  useEffect(() => {
    getMemory(id).then((value) => {
      setMemory(value); setLocation(value.place ?? ''); setParticipantIds(value.participantIds); setRestaurantStatus(value.restaurantStatus ?? 'visited'); setDescription(value.description ?? '');
      if (value.latitude !== undefined && value.longitude !== undefined) setPlace({ id: `memory-${value.id}`, label: value.place ?? value.title, name: value.title, latitude: value.latitude, longitude: value.longitude, city: value.city, country: value.country, cuisine: value.restaurantCuisine, website: value.restaurantWebsite, phone: value.restaurantPhone, openingHours: value.restaurantOpeningHours, source: value.restaurantSource === 'nominatim' || value.restaurantSource === 'photon' ? value.restaurantSource : undefined, sourceId: value.restaurantSourceId });
  ...
  return <main className="min-h-screen bg-zinc-950 px-4 py-6 text-white"><form onSubmit={submit} className="mx-auto max-w-2xl">
    ...
    <div className="mt-8 space-y-5 rounded-[2rem] border border-white/10 bg-white/[.03] p-6">
      ...
      <PlaceAutocomplete label={memory.isRestaurant?'Busca el restaurante':'Lugar'} kind={memory.isRestaurant?'restaurant':undefined} required value={location} onChange={(value) => { setLocation(value); setPlace(null); }} onSelect={setPlace} />
      ...
      {memory.isRestaurant&&<RestaurantGuideFields key={place?.id??'empty'} status={restaurantStatus} onStatusChange={setRestaurantStatus} defaults={place?.id===`memory-${memory.id}`?memory:undefined} place={place} onSuggestedDescription={setDescription}/>}
      <label className="block text-sm text-zinc-300">Notas<textarea name="description" value={description} onChange={(event)=>setDescription(event.target.value)} rows={6} className={`${field} h-auto py-3`} /></label>
