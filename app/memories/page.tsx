  ...
  return <main className="min-h-screen bg-zinc-950 px-4 py-6 text-zinc-100 sm:px-6 sm:py-8">
    <div className="mx-auto max-w-7xl">
      ...
      {archive.loading ? <div className="flex min-h-64 items-center justify-center gap-2 text-sm text-zinc-500"><Loader2 className="h-4 w-4 animate-spin"/>Cargando archivo…</div> : archive.error ? <div role="alert" className="mt-8 rounded-2xl border border-red-500/20 bg-red-500/5 p-6 text-center"><p className="text-sm text-red-200">{archive.error}</p><button onClick={() => void archive.refresh()} className="mt-4 inline-flex items-center gap-2 text-xs text-white"><RefreshCw className="h-3.5 w-3.5"/>Reintentar</button></div> : <>
        ...
        {showTrips && trips.length > 0 && <section className="mt-8"><SectionHeader eyebrow="Rutas" title="Viajes" count={trips.length}/><div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{trips.map((trip) => <Link href={`/trips/${trip.id}`} key={trip.id} className="group rounded-2xl border border-sky-500/15 bg-sky-500/[.04] p-5 transition hover:-translate-y-0.5 hover:border-sky-300/25 hover:bg-sky-500/[.07]"><div className="flex items-center justify-between"><span className="flex items-center gap-1.5 text-xs text-sky-300">{trip.transportMode === 'flight' ? <Plane className="h-3.5 w-3.5"/> : <Route className="h-3.5 w-3.5"/>}{trip.transportMode === 'flight' ? 'Vuelo' : 'Roadtrip'}</span><span className="text-xs text-zinc-600">{trip.stops.length} paradas</span></div><h3 className="mt-4 text-lg font-semibold">{trip.title}</h3><p className="mt-2 flex items-center gap-1.5 text-xs text-zinc-500"><CalendarDays className="h-3.5 w-3.5"/>{format(trip.startDate)} — {format(trip.endDate)}</p>{trip.cities.length>0&&<p className="mt-3 text-sm text-zinc-400">{trip.cities.join(' · ')}</p>}</Link>)}</div></section>}
