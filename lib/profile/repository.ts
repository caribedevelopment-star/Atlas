...
export async function getProfileSnapshot(requestedId?: string): Promise<ProfileSnapshot> {
  ...
  return { viewerId: auth.user.id, profile, access, statistics, memories, wines, libraryItems, countries, cities, favoritePlaces, favoriteWines, achievements: achievements(statistics), timeline: timeline(memories, wines, libraryItems) };
}
...
function timeline(memories: ProfileSnapshot['memories'], wines: WineItem[], library: ProfileSnapshot['libraryItems']): ProfileTimelineItem[] { return [...memories.map((item) => ({ id: `memory-${item.id}`, title: item.title, subtitle: item.place, date: item.date ?? item.createdAt, kind: 'memory' as const, href: `/memories/${item.id}` })), ...wines.map((item) => ({ id: `wine-${item.id}`, title: item.name, subtitle: item.winery, date: item.created_at, kind: 'wine' as const, href: `/wines/${item.id}` })), ...library.map((item) => ({ id: `library-${item.id}`, title: item.title, date: item.createdAt, kind: 'library' as const, href: `/library/${item.id}` }))].sort((a, b) => new Date(b.date ?? 0).getTime() - new Date(a.date ?? 0).getTime()).slice(0, 12); }
