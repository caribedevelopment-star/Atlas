...
export async function getAtlasMapSnapshot(): Promise<AtlasMapSnapshot> {
  ...
  authoritativeTrips.forEach((value) => {
    ...
    const trip = { ...value, source: source(value.userId, value.visibility, userId), points: routePoints, year: year(value.startDate), participantIds: value.participants.map((item) => item.id) };
  ...
  authorizedWines.forEach((wine) => wine.participants.forEach((person) => participantMap.set(person.id, person.name)));
