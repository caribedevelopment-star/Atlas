...
import { MapPinned, Plus, Route, Sparkles, Wine } from 'lucide-react';
...
export default function MapComponent() {
  ...
  return <section aria-label="Mapa del archivo Atlas" className="relative h-full min-h-[450px] overflow-hidden bg-zinc-950">
    ...
    {map.points.length === 0 ? <MapEmptyState reset={map.reset} /> : <div className="absolute bottom-[5.4rem] left-3 z-[690] max-w-[calc(100%-5.5rem)] sm:bottom-4 sm:left-5 sm:max-w-none">
      <div className="flex max-w-full items-center gap-1.5 overflow-x-auto rounded-[1.35rem] border border-white/12 bg-zinc-950/78 p-1.5 shadow-[0_18px_50px_rgba(0,0,0,.28)] backdrop-blur-2xl [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="flex shrink-0 items-center gap-2 rounded-2xl px-2.5 py-2 text-[10px] font-semibold uppercase tracking-[.14em] text-zinc-400"><span className="relative flex h-2 w-2"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-rose-400 opacity-45"/><span className="relative inline-flex h-2 w-2 rounded-full bg-rose-400"/></span>Atlas vivo</div>
        <LivePill icon={<MapPinned className="h-3.5 w-3.5"/>} label="Memorias" value={memories} onClick={() => map.applyPreset('all')} />
        <LivePill icon={<Wine className="h-3.5 w-3.5"/>} label="Vinos" value={wines} onClick={() => map.applyPreset('wine')} />
        <LivePill icon={<Route className="h-3.5 w-3.5"/>} label="Rutas" value={trips} onClick={() => map.applyPreset('trips')} />
        {map.snapshot.wineRegions.length > 0 && <LivePill icon={<Sparkles className="h-3.5 w-3.5"/>} label="DO" value={map.snapshot.wineRegions.length} onClick={() => map.applyPreset('wine')} accent />}
      </div>
    </div>}
