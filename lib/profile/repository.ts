...
async function profileAccess(viewerId: string, profile: AtlasProfile): Promise<ProfileAccess | null> {
  ...
  const { data, error } = await supabase.from('user_relationships').select('user_id,target_user_id,relationship').or(`and(user_id.eq.${viewerId},target_user_id.eq.${profile.id}),and(user_id.eq.${profile.id},target_user_id.eq.${viewerId})`);
  if (error) throw error;
  const accepted = (data ?? []).filter((row) => row.relationship === 'circle').length === 2;
...
export async function getProfileSnapshot(requestedId?: string): Promise<ProfileSnapshot> {
  ...
  const publicContributions = memories.filter((item) => item.visibility === 'public').length + wines.filter((item) => item.visibility === 'public').length + libraryItems.filter((item) => item.visibility === 'public').length;
...
async function countFriends(profileId: string) { const { data, error } = await supabase.from('user_relationships').select('user_id,target_user_id,relationship').or(`user_id.eq.${profileId},target_user_id.eq.${profileId}`).eq('relationship', 'circle'); if (error) throw error; const directions = new Map<string, number>(); (data ?? []).forEach((row) => { const other = row.user_id === profileId ? row.target_user_id : row.user_id; directions.set(other, (directions.get(other) ?? 0) + 1); }); return [...directions.values()].filter((count) => count >= 2).length; }
...
function achievements(stats: ProfileSnapshot['statistics']): ProfileAchievement[] { return [{ key: 'memories', title: 'Primera memoria', description: 'Guardó su primera memoria.' }, { key: 'wines', title: 'Primera botella', description: 'Añadió su primer vino.' }, { key: 'countriesVisited', title: 'Pasaporte Atlas', description: 'Registró su primer país visitado.' }, { key: 'publicContributions', title: 'Voz pública', description: 'Compartió su primera contribución pública.' }].filter((item) => stats[item.key as keyof typeof stats] !== null && Number(stats[item.key as keyof typeof stats]) > 0).map(({ key, ...item }) => ({ id: key, ...item })); }
