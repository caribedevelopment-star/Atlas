...
export default function NewMemoryPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === 'memory') {
        await createMemory({ title: String(data.get('title')), location: String(data.get('location')), date: String(data.get('date')), description: String(data.get('description')), visibility });
      } else {
...
function Stops({ stops, setStops }: { stops: Stop[]; setStops: React.Dispatch<React.SetStateAction<Stop[]>> }) { const update = (key: string, name: keyof Stop, value: string) => setStops((current) => current.map((stop) => stop.key === key ? { ...stop, [name]: value } : stop)); return <fieldset><div className="flex items-center justify-between"><legend className="text-sm font-medium text-zinc-300">Paradas ordenadas</legend><button type="button" onClick={() => setStops((current) => [...current, blankStop()])} className="flex items-center gap-1 text-xs text-rose-300"><Plus className="h-3.5 w-3.5"/>Parada</button></div><p className="mt-1 text-xs text-zinc-600">La línea seguirá exactamente este orden. No se infieren ubicaciones.</p><div className="mt-3 space-y-3">{stops.map((stop,index) => <div key={stop.key} className="rounded-2xl border border-white/10 bg-black/15 p-3"><div className="flex items-center gap-2"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white/10 text-xs text-zinc-300">{index + 1}</span><input value={stop.title} onChange={(event) => update(stop.key,'title',event.target.value)} placeholder="Nombre de la parada" className="h-10 min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-zinc-600"/>{stops.length > 2 && <button type="button" onClick={() => setStops((current) => current.filter((item) => item.key !== stop.key))} aria-label={`Eliminar parada ${index + 1}`} className="rounded-full p-2 text-zinc-600 hover:bg-white/5 hover:text-red-300"><Trash2 className="h-4 w-4"/></button>}</div><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">{(['city','country','latitude','longitude'] as const).map((name) => <input key={name} value={stop[name]} onChange={(event) => update(stop.key,name,event.target.value)} inputMode={name === 'latitude' || name === 'longitude' ? 'decimal' : undefined} placeholder={{ city:'Ciudad',country:'País',latitude:'Latitud',longitude:'Longitud' }[name]} className="h-10 min-w-0 rounded-xl border border-white/10 bg-white/[.035] px-3 text-xs text-white outline-none placeholder:text-zinc-600 focus:border-white/25"/>)}</div></div>)}</div></fieldset>; }
