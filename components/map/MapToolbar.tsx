...
import { CarFront, Check, Compass, Globe2, Heart, Layers3, Map, MapPinned, Plane, Route, Search, Ship, Sparkles, Store, TrainFront, Users, Wine, X } from 'lucide-react';
...
export function MapToolbar({ filters, snapshot, count, setQuery, setYear, setParticipant, toggleLayer, toggleSource, toggleTransport, toggleVisitedCountries, selectDenomination, applyPreset, reset }: { filters: AtlasMapFilters; snapshot: AtlasMapSnapshot; count: number; setQuery: (value: string) => void; setYear: (value: string) => void; setParticipant: (value: string) => void; toggleLayer: (value: MapLayer) => void; toggleSource: (value: MapSource) => void; toggleTransport: (value: TransportMode) => void; toggleVisitedCountries: () => void; selectDenomination: (id: string) => void; applyPreset: (preset: AtlasExplorePreset) => void; reset: () => void }) {
  ...
  const panelProps = { filters, snapshot, count, setQuery, setYear, setParticipant, toggleLayer, toggleSource, toggleTransport, toggleVisitedCountries, selectDenomination, applyPreset, reset };
  return <>
...
function PanelContent({ filters, snapshot, count, setQuery, setYear, setParticipant, toggleLayer, toggleSource, toggleTransport, toggleVisitedCountries, selectDenomination, applyPreset, reset }: { filters: AtlasMapFilters; snapshot: AtlasMapSnapshot; count: number; setQuery: (value: string) => void; setYear: (value: string) => void; setParticipant: (value: string) => void; toggleLayer: (value: MapLayer) => void; toggleSource: (value: MapSource) => void; toggleTransport: (value: TransportMode) => void; toggleVisitedCountries: () => void; selectDenomination: (id: string) => void; applyPreset: (preset: AtlasExplorePreset) => void; reset: () => void }) {
  ...
  return <div>
    ...
    {snapshot.visitedCountries.length>0&&<button type="button" onClick={toggleVisitedCountries} aria-pressed={filters.showVisitedCountries} className={`mt-3 flex w-full items-center gap-3 rounded-[1.15rem] border p-3 text-left transition ${filters.showVisitedCountries?'border-sky-200/30 bg-sky-300/[.1] shadow-[0_14px_35px_rgba(56,189,248,.08)]':'border-white/[.06] bg-white/[.02] hover:bg-white/[.05]'}`}><span className={`flex h-10 w-10 items-center justify-center rounded-2xl ${filters.showVisitedCountries?'bg-sky-200 text-sky-950':'bg-sky-300/[.08] text-sky-200'}`}><Globe2 className="h-4 w-4"/></span><span className="min-w-0 flex-1"><strong className="block text-xs font-medium text-white">Países visitados</strong><span className="mt-0.5 block text-[10px] text-zinc-600">Vista global · {snapshot.visitedCountries.length} {snapshot.visitedCountries.length===1?'país':'países'}</span></span><span className={`flex h-5 w-5 items-center justify-center rounded-full ${filters.showVisitedCountries?'bg-white text-zinc-950':'border border-white/10'}`}>{filters.showVisitedCountries&&<Check className="h-3 w-3"/>}</span></button>}
    ...
    {regions.length>0&&<section className="mt-5"><div className="flex items-center justify-between"><div><h3 className="text-[10px] font-semibold uppercase tracking-[.18em] text-zinc-500">Denominaciones</h3><p className="mt-1 text-[9px] text-zinc-700">Toca una región para acercarte; desde su ficha puedes abrir sus vinos.</p></div><span className="rounded-full border border-white/[.07] px-2 py-1 text-[9px] text-zinc-600">{snapshot.wineRegions.length}</span></div><div className="mt-2 grid grid-cols-2 gap-1.5">{regions.map((region)=>{const color=denominationStyle(region.country).accent;const active=filters.denominationId===region.id;return <button key={region.id} type="button" onClick={()=>selectDenomination(region.id)} aria-pressed={active} className={`group rounded-xl border p-2.5 text-left transition hover:-translate-y-0.5 ${active?'border-white/20 bg-white/[.1]':'border-white/[.06] bg-white/[.025] hover:bg-white/[.055]'}`} style={{boxShadow:active?`0 0 0 1px ${color}55,0 12px 30px ${color}18`:`inset 0 1px 0 rgba(255,255,255,.025)`}}><span className="mb-2 flex items-center justify-between"><span className="h-2 w-2 rounded-full" style={{backgroundColor:color,boxShadow:`0 0 12px ${color}77`}}/><span className="text-[7px] font-semibold uppercase tracking-[.13em]" style={{color}}>{region.classification||'GI'}</span></span><span className="block truncate text-[10px] font-medium text-white">{region.name}</span><span className="mt-1 block truncate text-[8px] uppercase tracking-wider text-zinc-600">{region.country} · {region.wineCount ? `${region.wineCount} vinos` : 'catálogo'}</span></button>})}</div></section>}
