...
export default function CaniaAssistant({ userWines }: CaniaAssistantProps) {
  ...
  async function sendMessage(raw: string) {
    ...
    try {
      const response = await fetch("/api/cania", {
        ...
        body: JSON.stringify({ message: userMsg, wines: userWines.slice(0, 100), history: messages.slice(-8) }),
      });
  ...
  return (
    <div className="fixed bottom-5 right-4 z-50 sm:bottom-6 sm:right-6">
      {!isOpen ? (
        <button aria-label="Abrir Can.iA" onClick={() => setIsOpen(true)} className="atlas-alive-button group flex items-center gap-2 rounded-full border border-rose-500/30 bg-zinc-950/95 px-4 py-3 text-rose-100 shadow-2xl shadow-black/40 backdrop-blur-xl transition duration-300 hover:-translate-y-0.5 hover:border-rose-400/50 hover:bg-zinc-900">
          <span className="relative flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-rose-500/25 to-amber-400/10 ring-1 ring-white/10">
            <Sparkles className="h-[18px] w-[18px] text-amber-300 transition group-hover:scale-110" />
          ...
          <span className="pr-1 text-left">
            ...
            <span className="block text-xs font-semibold text-white">Can.iA</span>
          </span>
        </button>
      ) : (
        <div className={`atlas-glass-panel flex flex-col overflow-hidden border border-white/10 bg-zinc-950/95 shadow-2xl shadow-black/50 backdrop-blur-2xl transition-all duration-300 ${expanded ? "h-[min(82dvh,760px)] w-[min(94vw,560px)] rounded-[2rem]" : "h-[min(72dvh,560px)] w-[min(92vw,390px)] rounded-[1.75rem]"}`}>
          <header className="flex items-center justify-between border-b border-white/10 bg-[radial-gradient(circle_at_20%_0%,rgba(244,63,94,.14),transparent_38%)] px-4 py-3.5">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2"><span className="truncate text-sm font-semibold text-white">Can.iA</span><span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide text-emerald-300">online</span></div>
                <p className="truncate text-[11px] text-zinc-500">Sommelier de Atlas · {userWines.length} vinos disponibles</p>
              </div>
            </div>
            <div className="flex items-center gap-1">
              <button type="button" onClick={() => setExpanded((value) => !value)} aria-label={expanded ? "Reducir Can.iA" : "Ampliar Can.iA"} className="rounded-xl p-2 text-zinc-500 transition hover:bg-white/5 hover:text-white">{expanded ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}</button>
              <button type="button" onClick={() => setIsOpen(false)} aria-label="Cerrar Can.iA" className="rounded-xl p-2 text-zinc-500 transition hover:bg-white/5 hover:text-white"><X className="h-4 w-4" /></button>
            </div>
          </header>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto px-3.5 py-4 text-sm [scrollbar-width:thin]">
            ...
            {loading && <div className="flex justify-start"><div className="flex items-center gap-1.5 rounded-2xl rounded-bl-md border border-white/10 bg-white/[.045] px-4 py-3 text-zinc-500"><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-rose-300 [animation-delay:-.2s]" /><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-rose-300 [animation-delay:-.1s]" /><span className="h-1.5 w-1.5 animate-bounce rounded-full bg-rose-300" /><span className="ml-1 text-[11px]">Can.iA está eligiendo…</span></div></div>}
            <div ref={endRef} />
          </div>

          <form onSubmit={handleSubmit} className="border-t border-white/10 bg-zinc-950/90 p-3"><div className="flex items-end gap-2 rounded-2xl border border-white/10 bg-white/[.045] p-1.5 transition focus-within:border-rose-400/30 focus-within:ring-4 focus-within:ring-rose-500/5"><input ref={inputRef} type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Pregunta por vino, comida o presupuesto…" className="min-w-0 flex-1 bg-transparent px-3 py-2.5 text-sm text-white outline-none placeholder:text-zinc-600" /><button type="submit" disabled={loading || !input.trim()} aria-label="Enviar mensaje" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-zinc-950 transition hover:bg-rose-50 active:scale-95 disabled:cursor-not-allowed disabled:opacity-30"><Send className="h-4 w-4" /></button></div><p className="mt-2 text-center text-[9px] text-zinc-700">Can.iA recomienda usando la bodega real de Atlas.</p></form>
        </div>
      )}
    </div>
  );
