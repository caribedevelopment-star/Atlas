...
export type AtlasExplorePreset = 'all' | 'mine' | 'wine' | 'trips' | 'restaurants' | 'shared' | 'public';

const defaultLayers: MapLayer[] = ['memories', 'wines', 'restaurants'];
...
export function useAtlasMap() {
  ...
  const points = useMemo(() => {
    ...
    return (snapshot?.points ?? []).filter((point) => filters.sources.has(point.source) && filters.layers.has(point.layer) && (!filters.year || point.year === filters.year) && (!filters.participant || point.participantIds.includes(filters.participant)) && (!query || [point.title, point.subtitle, point.ownerName, point.memory?.city, point.memory?.country, point.memory?.place, point.wine?.region, point.wine?.denomination, point.wine?.winery, ...point.participantNames].filter(Boolean).join(' ').toLocaleLowerCase('es').includes(query)));
  }, [filters, snapshot]);

  const wineRegions = useMemo(() => {
    if (!filters.layers.has('wines') || !filters.sources.has('public')) return [];
    const query = filters.query.trim().toLocaleLowerCase('es');
    return (snapshot?.wineRegions ?? []).filter((region) => !query || `${region.name} ${region.country} ${region.description} ${region.signature} ${region.grapes.join(' ')}`.toLocaleLowerCase('es').includes(query));
  ...
  const applyPreset = (preset: AtlasExplorePreset) => setFilters((current) => {
    ...
    if (preset === 'restaurants') return { ...base, sources: new Set<MapSource>(['mine', 'shared']), layers: new Set<MapLayer>(['restaurants']) };
    if (preset === 'shared') return { ...base, sources: new Set<MapSource>(['shared']), layers: new Set<MapLayer>(['memories', 'trips', 'restaurants']) };
