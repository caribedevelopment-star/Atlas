...
export async function getAtlasMapSnapshot(): Promise<AtlasMapSnapshot> {
  ...
  locatedMemories.forEach((memory) => {
    ...
    else points.push({ ...base, id: `memory-${memory.id}`, layer: 'memories' });
