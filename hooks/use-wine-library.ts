...
import { getCurrentWineUserId, listWines, setWineFavorite, updateWine } from '@/lib/wines/repository';
...
function isVisible(wine: WineItem, userId: string | null) {
  ...
  // RLS already authorized this friends-only row. Participants describe a
  // tasting and must not be treated as the friendship allow-list.
  if (wine.visibility === 'friends') return true;
  return true;
}
...
export function useWineLibrary() {
  ...
  const editWine=useCallback(async(wine:WineItem,changes:Parameters<typeof updateWine>[1])=>{const updated=await updateWine(wine.id,changes);setWines((current)=>current.map((item)=>item.id===wine.id?updated:item));setSelectedWine(updated);return updated;},[]);
  return { wines, filteredWines, loading, error, search, setSearch, filters, setFilters, sort, setSort, view, setView, selectedWine, setSelectedWine, options, refresh, toggleFavorite, addWine, editWine, clearError: () => setError(null), userId };
}
