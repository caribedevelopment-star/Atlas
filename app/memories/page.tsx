...
import { ArrowUpRight, Bookmark, CalendarDays, CarFront, Loader2, Map, MapPin, Plane, Plus, RefreshCw, Route, Search, Ship, Star, Store, TrainFront, Users } from 'lucide-react';
...
function MemoryCard({ memory }: { memory: ReturnType<typeof useMemoryArchive>['memories'][number] }) {
  ...
  return <article className={`group flex min-h-[250px] flex-col rounded-[1.55rem] border bg-zinc-900/45 p-5 transition hover:-translate-y-0.5 hover:bg-zinc-900/65 ${memory.isRestaurant?'border-emerald-300/[.12] hover:border-emerald-300/25':'border-white/[.08] hover:border-rose-300/15'}`}>
    ...
    <div className="mt-auto pt-5"><div className="flex items-center justify-between gap-3 border-t border-white/[.07] pt-4"><span className="flex min-w-0 items-center gap-1.5 text-[11px] text-zinc-600">{memory.participantIds.length ? <><Users className="h-3.5 w-3.5" /><span className="truncate">Con {memory.participantNames.join(', ')}</span></> : 'Solo tú'}</span><span className="flex items-center gap-1">{hasMap && <Link href={`/home?memory=${memory.id}`} aria-label={`Ver ${memory.title} en el mapa`} className="rounded-full border border-white/[.08] p-2 text-sky-300 transition hover:bg-white/[.07]"><Map className="h-3.5 w-3.5" /></Link>}<Link href={`/memories/${memory.id}`} aria-label={`Abrir ${memory.title}`} className="rounded-full border border-white/[.08] p-2 text-zinc-500 transition hover:bg-white hover:text-zinc-950"><ArrowUpRight className="h-3.5 w-3.5" /></Link></span></div></div>
  </article>;
}
