...
async function profileAccess(viewerId: string, profile: AtlasProfile): Promise<ProfileAccess | null> {
  ...
  const { data, error } = await supabase.from('friendships').select('id').eq('status','accepted').or(`and(requester_id.eq.${viewerId},addressee_id.eq.${profile.id}),and(requester_id.eq.${profile.id},addressee_id.eq.${viewerId})`);
  if (error) throw error;
  const accepted = Boolean(data?.length);
...
async function countFriends(profileId: string) { const { count, error } = await supabase.from('friendships').select('id',{count:'exact',head:true}).eq('status','accepted').or(`requester_id.eq.${profileId},addressee_id.eq.${profileId}`); if (error) throw error; return count??0; }
