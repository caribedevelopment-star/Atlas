...
import type { AtlasMapPoint, AtlasWineRegion, MapCoordinate, MapLayer, MapTrip } from '@/types/map';
...
export function AtlasLeafletMap({ points, wineRegions = [] }: { points: AtlasMapPoint[]; wineRegions?: AtlasWineRegion[] }) {
  ...
  return <MapContainer center={[20, 0]} zoom={3} minZoom={2} scrollWheelZoom className="h-full w-full bg-[#d9d8d3]" zoomControl={false} preferCanvas={false}>
    ...
    <MarkerClusterGroup chunkedLoading chunkInterval={100} chunkDelay={20} removeOutsideVisibleBounds spiderfyOnMaxZoom>
      {markers.map((point) => <Marker key={point.id} position={[point.latitude, point.longitude]} icon={icon(point.layer)} title={point.title} keyboard>
...
function RoadTripRoute({ point }: { point: AtlasMapPoint }) {
  ...
  useEffect(() => {
    ...
    void getRoadRoute(seed, controller.signal).then((next) => {
      if (next.length > 1) setRoute(next);
    }).catch(() => undefined);
  ...
  return <>
    ...
    <Polyline positions={positions} pathOptions={{ color, weight: 4.2, opacity: .78, lineCap: 'round', lineJoin: 'round' }}>
      <Popup className="atlas-map-popup"><TripPopup trip={trip} /></Popup>
    </Polyline>
...
function LivingWineRegion({ region }: { region: AtlasWineRegion }) {
  ...
  return <>
    ...
    <Circle center={[region.latitude, region.longitude]} radius={Math.round(region.radius * 1.08)} pathOptions={{ color, fillColor: color, fillOpacity: .015, opacity: .42, weight: 1.6, dashArray: '2 11', className: 'atlas-do-pulse' }}>
      <Tooltip direction="top" sticky opacity={.96}>
        <div className="min-w-[150px] py-1">
          <strong>{region.name}</strong><br />
          <span>{region.country}</span><br />
          <span>{region.wineCount} {region.wineCount === 1 ? 'vino' : 'vinos'} · {region.wineryCount} {region.wineryCount === 1 ? 'bodega' : 'bodegas'}</span>
          {region.averageRating ? <><br /><span>★ {region.averageRating}/5</span></> : null}
        </div>
      </Tooltip>
    </Circle>
    <CircleMarker center={[region.latitude, region.longitude]} radius={4 + Math.min(4, region.wineCount * .65)} pathOptions={{ color: '#fff', fillColor: color, fillOpacity: .94, weight: 2, className: 'atlas-do-core' }}>
...
function CurrentLocation({ focus }: { focus: boolean }) {
  ...
  return position ? <><Circle center={position} radius={120} pathOptions={{ color: '#007aff', fillColor: '#007aff', fillOpacity: .08, weight: 1 }} /><CircleMarker center={position} radius={8} pathOptions={{ color: '#fff', fillColor: '#007aff', fillOpacity: 1, weight: 3 }}><Tooltip direction="top" offset={[0, -8]}>{label}</Tooltip></CircleMarker></> : null;
}

function validCoordinate(latitude?: number, longitude?: number) {
  return latitude !== undefined && longitude !== undefined && Number.isFinite(latitude) && Number.isFinite(longitude) && latitude >= -90 && latitude <= 90 && longitude >= -180 && longitude <= 180;
}

function routeColor(ownerId: string, source: AtlasMapPoint['source']) {
  if (source === 'mine') return '#0A84FF';
  const palette = source === 'friends' ? ['#64D2FF', '#BF5AF2', '#30D158', '#FF9F0A', '#FF375F'] : ['#5E5CE6', '#32ADE6', '#AC8E68', '#8E8E93'];
  let hash = 0;
  for (let index = 0; index < ownerId.length; index += 1) hash = ((hash << 5) - hash + ownerId.charCodeAt(index)) | 0;
  return palette[Math.abs(hash) % palette.length];
}

function wineRegionColor(name: string) {
  const palette = ['#9F1239', '#7C3AED', '#B45309', '#0F766E', '#BE123C', '#6D28D9', '#A16207'];
  let hash = 0;
...
function icon(layer: MapLayer) {
  const existing = cache.get(layer); if (existing) return existing;
  const colors: Record<MapLayer, string> = { memories: '#ff375f', wines: '#7c2d12', trips: '#007aff', favorites: '#ff9f0a', restaurants: '#30d158' };
  const size = layer === 'wines' ? 18 : 28;
  const value = L.divIcon({ className: `atlas-map-marker atlas-map-marker-${layer}`, html: `<span style="background:${colors[layer]};width:${size}px;height:${size}px"></span>`, iconSize: [size, size], iconAnchor: [size / 2, size / 2], popupAnchor: [0, -size / 2] });
  cache.set(layer, value); return value;
}
