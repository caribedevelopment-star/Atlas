  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { reply: 'Hola, falta configurar la clave GEMINI_API_KEY en las variables de entorno de Vercel.' },
        { status: 200 }
      );
    }
...
- Si no tiene ninguno adecuado en su lista, recomiéndale uno clásico de mercado y explícale por qué.
- Mantén las respuestas claras y no demasiado largas.
    `;

    // Endpoint oficial v1 para gemini-1.5-flash
    ...
    if (data.error) {
      console.error('Error detallado de la API de Gemini:', data.error);
      return NextResponse.json({ 
        reply: 'Disculpa, tuve un problema al consultar mi manual de cata. Inténtalo de nuevo.' 
      });
    }
