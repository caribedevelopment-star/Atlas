...
export function useAtlasMap() {
  ...
  return { snapshot, points, filters, setQuery: (query: string) => setFilters((current) => ({ ...current, query })), setYear: (year: string) => setFilters((current) => ({ ...current, year })), setParticipant: (participant: string) => setFilters((current) => ({ ...current, participant })), toggleSource, toggleLayer, reset: () => setFilters(initialFilters), loading, error, refresh };
}
