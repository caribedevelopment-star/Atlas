...
import { normalizeTransportMode } from '@/lib/trips/transport';
...
export function normalizeTrip(row: Row): AtlasTrip {
  ...
  return { id:String(row.id),userId:String(row.user_id),title:String(row.title),description:text(row.description),coverImageUrl:text(row.cover_image_url),galleryUrl:text(row.gallery_url),startDate:String(row.start_date),endDate:String(row.end_date),transportMode:normalizeTransportMode(row.transport_mode,row.title,row.description),visibility:visibility(row.visibility),routeGeometry:points,participants,stops,wines,photos,countries:unique(stops.map((stop)=>stop.country)),cities:unique(stops.map((stop)=>stop.city)),distanceKm:distance(points),createdAt:text(row.created_at),updatedAt:text(row.updated_at) };
}
export async function listTrips():Promise<AtlasTrip[]> { const {data,error}=await supabase.from('trips').select(query).order('start_date',{ascending:false}); if(error) throw error; return hydrateTrips((data??[]).map((row)=>normalizeTrip(row as Row))); }
export async function getTrip(id:string):Promise<AtlasTrip> { const {data,error}=await supabase.from('trips').select(query).eq('id',id).single(); if(error) throw error; return (await hydrateTrips([normalizeTrip(data as Row)]))[0]; }
export async function saveTrip(input:TripInput):Promise<string> { const routeGeometry=input.stops.flatMap((stop)=>stop.latitude!==undefined&&stop.longitude!==undefined?[[stop.longitude,stop.latitude]]:[]); const {data,error}=await supabase.rpc('atlas_save_trip',{trip_id:input.id??null,payload:{...input,visibility:input.participantIds.length&&input.visibility==='private'?'friends':input.visibility,routeGeometry:{type:'LineString',coordinates:routeGeometry}}}); if(error){if(error.code==='PGRST202')throw new Error('La función de viajes no está instalada en Supabase. Aplica la migración Phase 8.');throw new Error(error.message||'No se pudo guardar el viaje.')} if(!data)throw new Error('Supabase no devolvió el viaje guardado.'); if(input.galleryUrl){const {error:galleryError}=await supabase.from('trips').update({gallery_url:input.galleryUrl}).eq('id',data);if(galleryError)throw new Error('El viaje se guardó, pero no se pudo vincular la galería. Aplica la migración de galería.');} return data as string; }
...
async function hydrateTrips(trips: AtlasTrip[]): Promise<AtlasTrip[]> {
  const ids = [...new Set(trips.flatMap((trip) => [trip.userId, ...trip.participants.map((participant) => participant.id)]))];
  ...
  return trips.map((trip) => ({
    ...trip,
    ownerName: profiles.get(trip.userId)?.name,
    ownerAvatarUrl: profiles.get(trip.userId)?.avatarUrl,
    participants: trip.participants.map((participant) => ({ ...participant, name: profiles.get(participant.id)?.name ?? participant.name, avatarUrl: profiles.get(participant.id)?.avatarUrl ?? participant.avatarUrl })),
  }));
}
