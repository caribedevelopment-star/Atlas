...
import { BookOpen, MapPinned, Orbit, Plane, Sparkles, Users, Wine, X } from 'lucide-react';
...
export function AtlasUniverse({ snapshot, onClose }: { snapshot: AtlasMapSnapshot; onClose: () => void }) {
  ...
  const stats = useMemo(() => {
    ...
    const shared = snapshot.points.filter((point) => point.source === 'shared' && (point.layer === 'memories' || point.layer === 'trips'));
    const countries = new Set<string>();
    const cities = new Set<string>();

    memories.forEach((point) => {
    ...
    return [
      ...
      { label: 'Viajes', value: trips.length, detail: 'trazados', icon: Orbit },
      { label: 'Vinos', value: wines.length, detail: 'tuyos', icon: Wine },
  ...
  return <div className="atlas-universe-enter fixed inset-0 z-[4000] overflow-hidden bg-[#020205] text-white" role="dialog" aria-modal="true" aria-label="Tu universo Atlas">
    ...
    <main className="relative flex h-full min-h-[620px] items-center justify-center px-4 py-24">
      <div className="atlas-universe-system relative h-[min(78vw,720px)] w-[min(92vw,820px)] max-h-[72dvh] min-h-[480px] min-w-[340px]">
        <div className="atlas-universe-orbit atlas-universe-orbit-a absolute left-1/2 top-1/2 h-[82%] w-[82%] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/[.07]" />
        <div className="atlas-universe-orbit atlas-universe-orbit-b absolute left-1/2 top-1/2 h-[62%] w-[62%] -translate-x-1/2 -translate-y-1/2 rounded-full border border-dashed border-cyan-200/[.09]" />
        <div className="atlas-universe-orbit atlas-universe-orbit-c absolute left-1/2 top-1/2 h-[44%] w-[44%] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/[.06]" />

        <div className="atlas-universe-core absolute left-1/2 top-1/2 z-10 flex h-[190px] w-[190px] -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-full text-center sm:h-[230px] sm:w-[230px]">
          <span className="atlas-universe-core-glow absolute -inset-12 rounded-full" />
          <span className="relative flex h-12 w-12 items-center justify-center rounded-2xl border border-white/15 bg-white/[.07] shadow-2xl backdrop-blur-xl"><Orbit className="h-6 w-6 text-cyan-100"/></span>
          <strong className="relative mt-4 text-3xl font-semibold tracking-[-.06em] sm:text-4xl">Atlas</strong>
          <span className="relative mt-1 text-xs text-white/45">Tu vida, vista desde fuera.</span>
        </div>
    ...
    <div className="absolute inset-x-4 bottom-5 z-30 mx-auto flex max-w-lg items-center justify-center gap-2 rounded-[1.4rem] border border-white/[.08] bg-black/25 px-4 py-3 text-center text-[11px] text-white/40 shadow-2xl backdrop-blur-2xl sm:bottom-7"><BookOpen className="h-3.5 w-3.5 text-white/35"/><span>Este universo se construye solo con tu Atlas y lo que otros han compartido contigo.</span></div>
  </div>;
}
...
function UniverseStat({ label, value, detail, icon: Icon, index, total }: { label: string; value: number; detail: string; icon: typeof Orbit; index: number; total: number }) {
  ...
  return <div className="atlas-universe-stat absolute z-20 -translate-x-1/2 -translate-y-1/2" style={{ left: `${left}%`, top: `${top}%`, animationDelay: `${index * -0.7}s`, ['--atlas-stat-scale' as string]: String(.9 + (depth + 1) * .08) }}>
    <div className="min-w-[112px] rounded-[1.35rem] border border-white/[.1] bg-white/[.055] p-3 text-center shadow-[0_18px_55px_rgba(0,0,0,.35)] backdrop-blur-2xl transition duration-300 hover:-translate-y-1 hover:border-white/20 hover:bg-white/[.085] sm:min-w-[128px] sm:p-4">
      <span className="mx-auto flex h-8 w-8 items-center justify-center rounded-xl border border-white/10 bg-white/[.05] text-cyan-100"><Icon className="h-3.5 w-3.5"/></span>
      <strong className="mt-2 block text-2xl font-semibold tracking-[-.04em] sm:text-3xl">{value}</strong>
      <span className="mt-0.5 block text-[10px] font-semibold uppercase tracking-[.14em] text-white/60">{label}</span>
