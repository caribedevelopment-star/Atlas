...
import { Calendar, Edit3, Eye, Heart, Loader2, MapPin, ShoppingBag, Star, X } from 'lucide-react';
import type { CreateWineInput, WineItem } from '@/types/wine';
import { WineBottleImage } from './WineBottleImage';

export function WineDetailDialog({ wine, editable=false, onClose, onFavorite, onSave }: { wine: WineItem | null;editable?:boolean; onClose: () => void; onFavorite: (wine: WineItem) => void;onSave:(wine:WineItem,changes:Partial<CreateWineInput>)=>Promise<WineItem> }) {
  const closeRef = useRef<HTMLButtonElement>(null);
  const[editing,setEditing]=useState(false),[saving,setSaving]=useState(false),[error,setError]=useState<string|null>(null);
  useEffect(() => {
    if (!wine) return;
  ...
  if (!wine) return null;
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!wine)return;setSaving(true);setError(null);const data=new FormData(event.currentTarget);try{await onSave(wine,{name:String(data.get('name')),winery:String(data.get('winery')),rating:Number(data.get('rating'))||undefined,tasting_notes:String(data.get('notes'))});setEditing(false);}catch(cause){setError(cause instanceof Error?cause.message:'No se pudo guardar.');}finally{setSaving(false);}}
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section role="dialog" aria-modal="true" aria-labelledby="wine-detail-title" className="max-h-[94dvh] w-full max-w-4xl overflow-y-auto rounded-t-[2rem] border border-white/10 bg-zinc-950 shadow-2xl sm:rounded-[2rem]">
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/10 bg-zinc-950/90 px-5 py-4 backdrop-blur-xl"><p className="text-sm text-zinc-400">Ficha de la botella</p><div className="flex gap-2">{editable&&<button onClick={()=>setEditing(!editing)} className="rounded-full p-2 text-zinc-400 hover:bg-white/10" aria-label="Editar vino"><Edit3 className="h-5 w-5"/></button>}<button ref={closeRef} onClick={onClose} className="rounded-full p-2 text-zinc-400 hover:bg-white/10 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-400" aria-label="Cerrar detalle"><X className="h-5 w-5" aria-hidden="true" /></button></div></div>
      <div className="grid gap-7 p-5 sm:grid-cols-[minmax(240px,0.8fr)_1.2fr] sm:p-7">
        ...
        <div>{editing?<form onSubmit={submit} className="space-y-4"><EditField name="name" label="Nombre" defaultValue={wine.name}/><EditField name="winery" label="Bodega" defaultValue={wine.winery}/><EditField name="rating" label="Valoración" type="number" min="0" max="5" step="0.5" defaultValue={wine.rating}/><label className="block text-sm text-zinc-400">Notas<textarea name="notes" defaultValue={notes} rows={5} className="mt-1 w-full rounded-xl border border-white/10 bg-zinc-900 p-3 text-white"/></label>{error&&<p className="text-sm text-red-300">{error}</p>}<button disabled={saving} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-white font-semibold text-zinc-950">{saving&&<Loader2 className="h-4 w-4 animate-spin"/>}Guardar cambios</button></form>:<><div className="flex items-start justify-between gap-3"><div><p className="text-sm text-rose-300">{wine.winery || 'Bodega sin especificar'}</p><h2 id="wine-detail-title" className="mt-1 text-3xl font-semibold tracking-tight text-white">{wine.name}</h2></div><button onClick={() => onFavorite(wine)} aria-pressed={wine.favorite} aria-label={wine.favorite ? 'Quitar de favoritos' : 'Añadir a favoritos'} className="rounded-full border border-white/10 p-3 text-zinc-400 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-400"><Heart className={`h-5 w-5 ${wine.favorite ? 'fill-rose-500 text-rose-500' : ''}`} aria-hidden="true" /></button></div>
...
function EditField(props:React.InputHTMLAttributes<HTMLInputElement>&{label:string}){const{label,...input}=props;return <label className="block text-sm text-zinc-400">{label}<input {...input} className="mt-1 h-11 w-full rounded-xl border border-white/10 bg-zinc-900 px-3 text-white"/></label>}
